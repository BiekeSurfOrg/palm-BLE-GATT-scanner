...
async def _perform_ble_scan_core() -> Dict[str, str]:
    ...
    logging.info("Starting BLE scan process.")

    # 1) macOS: ensure Bluetooth is available; if not, return a friendly status.
    if MAC:
        start_bt_check = time.time()
        ok = await _wait_bluetooth_available(total=30.0, step=2.0)
        end_bt_check = time.time()
        logging.info(f"Bluetooth availability check took {end_bt_check - start_bt_check:.2f} seconds.")
        if not ok:
            return {
    ...
    try:
        ...
        logging.error(f"BleakError during scan: {e}")
        return {"status": "Bluetooth error", "info": f"{e}"}
    except Exception as e:
        logging.error(f"Unexpected error during scan: {e}")
        return {"status": "Bluetooth error", "info": f"{e}"}
    end_scan_time = time.time()
    logging.info(f"BLE scan took {end_scan_time - start_scan_time:.2f} seconds.")

    if not target:
        logging.info("No target BLE package found.")
        return {"status": "No BLE package found", "info": ""}

    logging.info(f"Device '{target['name']}' ({target['address']}) found with RSSI {target['rssi']}.")
    ...
    try:
        async with BleakClient(target["address"], timeout=CONNECT_TIMEOUT_S) as client:
            ...
            if not client.is_connected:
                logging.error(f"Failed to connect to {target['address']}")
            ...
            start_service_discovery_time = time.time()
            try:
            ...
            try:
                ...
                if not target_characteristic_obj:
                    ...
                    end_notify_time = time.time()
                    logging.info(f"Notification start to stop took {end_notify_time - start_notify_time:.2f} seconds.")
                # <<< MODIFICATION END >>>
